...
import { dealsViewProductPageData } from '../../data/productsDealsViewProductPage';
import 'swiper/css';
import 'swiper/css/navigation';
import 'swiper/css/thumbs';
import './DealsProductPage.css';

const DealsViewProductPage = () => {
  const [selectedColor, setSelectedColor] = useState(0);
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  const { id } = useParams();
  const navigate = useNavigate();
  const productData = dealsViewProductPageData[id];

  useEffect(() => {
    console.log('Current ID:', id);
    console.log('Available IDs:', Object.keys(dealsViewProductPageData));
    console.log('Product Data:', productData);
    
    if (!productData || !productData.colors || productData.colors.length === 0) {
      console.log('Product not found or invalid data, navigating to 404');
      navigate('/404');
    }
...
export default DealsViewProductPage;
