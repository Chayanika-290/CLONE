...
import { newLaunchesProductData, personalisationProductData } from '../../data/productsTopPicksProductPage';

const TopPicksProductPage = () => {
  const [selectedColor, setSelectedColor] = useState(0);
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  const { category, id } = useParams();
  const navigate = useNavigate();
  
  const productData = category === 'new-launches' 
    ? newLaunchesProductData[id]
    : personalisationProductData[id];
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-6 py-8">
      ...
      <div className="flex items-center gap-2 text-sm mb-8">
        ...
        <span>Top Picks</span>
        <span>/</span>
        <span>{category === 'new-launches' ? 'New Launches' : 'Personalisation'}</span>
        <span>/</span>
...
export default TopPicksProductPage;
