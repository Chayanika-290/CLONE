...
import { topPicksViewProductData } from '../../data/productsTopPicksViewProductPage';

const TopPicksViewProductPage = () => {
  const [selectedColor, setSelectedColor] = useState(0);
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  const { id } = useParams();
  const navigate = useNavigate();
  const productData = topPicksViewProductData[id];
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-6 py-8">
      ...
      <div className="flex items-center gap-2 text-sm mb-8">
        ...
        <span>/</span>
        <Link to="/top-picks" className="hover:text-red-500">All Products</Link>
        <span>/</span>
...
export default TopPicksViewProductPage;
